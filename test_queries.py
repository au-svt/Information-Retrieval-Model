...
def get_normalized_query_scores(query_terms, freq_list, inverted_index):
    ...
    for term in query_terms:
        tf_weights[term] = 1 + math.log10(query_terms[term])	#query tf
...
def get_normalized_doc_weights(query_terms, freq_list, inverted_index):
    ...
    for i in range(len(freq_list)):
        for term in freq_list[i].keys():
            ...
            # doc_weights[i] is: list of term and its unigram score for ith document in the log and both of them are stored as a pair
    ...
    for i in range(len(doc_weights)):
        ...
        # doc_tf is a list for a document, doc_tf[i] is [ith term, score] 
        for j in range(len(doc_tf)):
            normalized_doc_weights[i].append([doc_tf[j][0], doc_tf[j][1]*factor])
        # normalized the scores
    
    return normalized_doc_weights	# normalized_doc_weights[1] = [['the', 0.4], ...]
...
def search(query, inverted_index, freq, title_list):
    query_processed = query_pre_process(query)	# list of tokens
    query_terms = get_query_terms(query_processed)	# dictionary
    print("-"*50)
    print("Query Terms: ", query_terms)

    query_wt = get_normalized_query_scores(query_terms, freq, inverted_index) #dictionary of normalized scores
    document_wt = get_normalized_doc_weights(query_terms, freq, inverted_index) # normalized_doc_weights[1] = [['the', 0.4], ...]

    scores = compute_scores(query_wt, document_wt)	# returns the list of scores, scores[1] = [1,0.45]
...
def main():
    ...
    while 1:
        ...
        if option=='1' :
            ...
            x = input("Which one you wanna go with, \n\t1.fast?(only 3 queries will run from ['Main cause of poverty', 'Enlighten me!', 'Viral Foreigner'] ): Enter 1, or \n\t2.slow? Enter 2\n")
            if(x=='2') : 
